...
Public Class DashboardPayment
    ...
    Sub NewIssueInvoice()
        ...
        IsInvoice.LblDrName.Text = IIf(TypeOf GridPatientWaiting.GetRow.Cells("OPHTHAMOLOGIST").Value Is DBNull, "", GridPatientWaiting.GetRow.Cells("OPHTHAMOLOGIST").Value)
        IsInvoice.LblDiagnosis.Text = IIf(TypeOf GridPatientWaiting.GetRow.Cells("DIAGNOSIS").Value Is DBNull, "", GridPatientWaiting.GetRow.Cells("DIAGNOSIS").Value)
    ...
    Private Sub BtnPrescription_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrescription.Click
        Try
            'GenerateReportKhmer("select * from V_EYE_PRESCRIPTION WHERE WAIT_ID =" & GridPatientWaiting.GetRow.Cells("WAIT_ID").Value, Application.StartupPath & "\CrpPrescriptionV1.rpt")
